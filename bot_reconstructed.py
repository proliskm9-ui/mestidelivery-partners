        ...
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    ...
                    web_app=WebAppInfo(url=f"{MINIAPP_URL}?order_id={order_id}"),
                )
            ]
        ]
    )
...
def format_order_message(data: dict) -> str:
    """Форматирует красивое HTML-сообщение о новом заказе (только состав, сумма без доставки, коммент)."""
    ...
    return (
        ...
        f"{items_text}"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💰 <b>Сумма заказа:</b> <b>{food_total:.0f} {currency}</b>"
        f"{comment_block}"
    )
...
I18N = {
    "ru": {
        "welcome": "👋 Welcome, {first_name}!\n\n💼 This is the bot for MestiDelivery partners. Here you will be able to receive order notifications and manage deliveries.\nTo continue, choose your language 👇",
        ...
        "status_online_now": "✅ Вы вышли на линию! Теперь вы будете получать заказы.",
        "status_offline_now": "❌ Вы сошли с линии. Заказы временно не будут поступать."
    },
    "en": {
        ...
        "status_online_now": "✅ You are online! You will now receive orders.",
        "status_offline_now": "❌ You are offline. You will temporarily not receive orders."
    },
    "ka": {
        ...
        "menu_courier_online": "🟢 ხაზზე გასვლა",
        "menu_courier_offline": "🔴 ხაზიდან გამოსვლა",
        "status_online_now": "✅ თქვენ ხაზზე ხართ! ახლა მიიღებთ შეკვეთებს.",
        "status_offline_now": "❌ თქვენ ხაზიდან გამოხვედით. შეკვეთებს დროებით არ მიიღებთ."
    }
}
...
def get_admin_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            ...
            InlineKeyboardButton(text="🍽️ Рестораны", callback_data="admin_restaurants"),
        ],
    ])
...
async def build_admin_text() -> str:
    ...
    couriers_total = stats["couriers_total"]
    couriers_online = stats["couriers_online"]
    restaurants_total = stats["restaurants_total"]
    ...
    text = (
        f"🛡️ <b>Панель администратора MestiDelivery</b>\n"
        f"🕐 Обновлено: {now}\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🏍️ <b>Курьеры:</b>\n"
        f"   Всего зарегистрировано: <b>{couriers_total}</b>\n"
        f"   🟢 Сейчас на линии: <b>{couriers_online}</b>\n"
        f"   🔴 Офлайн: <b>{couriers_total - couriers_online}</b>\n\n"
        f"   Онлайн:\n{online_str}\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🍽️ <b>Рестораны-партнёры:</b> <b>{restaurants_total}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━"
    )
    return text


async def send_admin_dashboard(chat_id: int):
    text = await build_admin_text()
    await bot.send_message(chat_id, text, parse_mode=ParseMode.HTML, reply_markup=get_admin_kb())


@router.callback_query(F.data == "admin_refresh")
async def on_admin_refresh(callback: CallbackQuery):
    if callback.from_user.id != ADMIN_TG_ID:
        await callback.answer("Доступ запрещён", show_alert=True)
        return
    text = await build_admin_text()
    try:
        await callback.message.edit_text(text, parse_mode=ParseMode.HTML, reply_markup=get_admin_kb())
    except Exception:
        pass
...
async def on_admin_couriers(callback: CallbackQuery):
    if callback.from_user.id != ADMIN_TG_ID:
        await callback.answer("Доступ запрещён", show_alert=True)
        return
    online_ids = await db.get_online_courier_ids()
    all_ids = await db.get_all_courier_ids()

    lines = []
    for tg_id in all_ids:
        user = await db.get_bot_user(tg_id)
        name = (user or {}).get("backend_username", f"id:{tg_id}")
        status = "🟢" if tg_id in online_ids else "🔴"
        lines.append(f"{status} {name} (tg: {tg_id})")

    text = "👥 <b>Все курьеры:</b>\n\n" + ("\n".join(lines) if lines else "Нет зарегистрированных курьеров")
    await callback.message.answer(text, parse_mode=ParseMode.HTML)
    await callback.answer()


@router.callback_query(F.data == "admin_restaurants")
async def on_admin_restaurants(callback: CallbackQuery):
    if callback.from_user.id != ADMIN_TG_ID:
        await callback.answer("Доступ запрещён", show_alert=True)
        return
    async with db._db.execute(
        "SELECT b.telegram_id, b.backend_username, p.restaurant_name FROM bot_users b "
        "LEFT JOIN partners p ON b.telegram_id = p.telegram_id "
        "WHERE b.role = 'restaurant_admin'"
    ) as cur:
        rows = await cur.fetchall()

    if not rows:
        await callback.message.answer("🍽️ Нет зарегистрированных ресторанов")
    else:
        lines = []
        for row in rows:
            name = row[2] or row[1] or f"id:{row[0]}"
            lines.append(f"🍽️ {name} (tg: {row[0]})")
        text = "🍽️ <b>Рестораны-партнёры:</b>\n\n" + "\n".join(lines)
        await callback.message.answer(text, parse_mode=ParseMode.HTML)
    await callback.answer()
...
async def cmd_reset(message: Message, state: FSMContext):
    ...
    await message.answer("🔄 <b>Ваш профиль в боте сброшен!</b>\nОтправьте `/start` для новой регистрации.")
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if user:
        ...
        if lang == "ru":
            caption = f"🚀 <b>Добро пожаловать в панель партнёра, {pname}!</b>\n\n"
            caption += "Здесь вы можете управлять вашим бизнесом прямо из Telegram. "
            caption += "Отслеживайте новые заказы, просматривайте историю и статистику в один клик.\n\n"
            caption += "👇 <i>Используйте меню ниже для навигации:</i>"
        elif lang == "en":
            caption = f"🚀 <b>Welcome to the partner dashboard, {pname}!</b>\n\n"
            caption += "Here you can manage your business directly from Telegram. "
            caption += "Track new orders, view history and statistics in one click.\n\n"
            caption += "👇 <i>Use the menu below to navigate:</i>"
        else:
            caption = f"🚀 <b>კეთილი იყოს თქვენი მობრძანება პარტნიორის პანელში, {pname}!</b>\n\n"
            caption += "აქ შეგიძლიათ მართოთ თქვენი ბიზნესი პირდაპირ Telegram-იდან.\n\n"
            caption += "👇 <i>ნავიგაციისთვის გამოიყენეთ ქვემოთ მოცემული მენიუ:</i>"
...
async def main():
    ...
    # Сброс всех курьеров в офлайн при каждом запуске.
    # Курьеры должны явно нажать «Выйти на линию», чтобы получать заказы.
    await db._db.execute("UPDATE bot_users SET is_online = 0 WHERE role = 'courier'")
    await db._db.commit()
    log.info("All couriers reset to offline on startup")
